...
def use_server_state(key: str, initial_value: StateValueT) -> ServerStateItem[StateValueT]:
    server = Server.get_current()

    attr_name = f"{SERVER_STATE_KEY_PREFIX}{key}"

    server_state: ServerStateItem[StateValueT]
    if not hasattr(server, attr_name):
        server_state = ServerStateItem(initial_value)
